    ...
    methods
        ...
        function [contents,messages,batchLines] = parseBatchOutput(~,rawOutput)
            ...
            for i = 1:numel(lines)
                ...
                if isfield(batchLine, "custom_id")
                    customId = string(batchLine.custom_id);
                else
                    continue
                end
        
                if ~isfield(batchLine, "response") || isempty(batchLine.response) || ...
                   ~isfield(batchLine.response, "body") || ...
                   ~isfield(batchLine.response.body, "choices") || ...
                   isempty(batchLine.response.body.choices)
                ...
                choice = batchLine.response.body.choices(1);
                if ~isfield(choice, "finish_reason") || ~isfield(choice, "message")
                    contents(end+1) = struct( ...
                        "custom_id", customId, ...
                        "finish_reason", "error", ...
                        "content", "", ...
